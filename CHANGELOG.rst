...
- Fix well-formedness detection for user-defined ``text/plain`` files due to File/magic detecting an unsupported file format such as ``application/SIMH-tape-data``
...
Fixed
^^^^^
- Fix scraping WARC files with non-UTF-8 characters
...
Changed
^^^^^^^
- Added file format and version detection of gzipped WARC files without performing well-formedness check
...
Added
^^^^^
- File format and stream detection for Dolby Digital AC-3 audio
